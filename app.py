...
============================
Interactive dashboard to visualize everything we built.

Run with:
    streamlit run app.py

Features:
    - Simulate new stock paths live
...
# ──────────────────────────────────────────────────────
st.set_page_config(
    page_title="RL Derivative Hedging",
    page_icon="📈",
    layout="wide",
...
st.markdown(
    "An interactive dashboard exploring how a PPO RL agent learns to hedge options "
    "better than naive delta hedging."
)

# ──────────────────────────────────────────────────────
# HELPER FUNCTIONS (same as Phase 1, inline for dashboard)
# ──────────────────────────────────────────────────────
...
def simulate_rl_agent(delta_series):
    """
    Simulate a simple RL-like agent that tracks delta but with noise reduction.
    (In production, replace with model.predict())
    Uses exponential smoothing to mimic what a trained RL agent learns:
    hold a smoothed version of delta to reduce transaction costs.
    """
    ...
    # Add small random exploration noise to simulate real RL
...
st.sidebar.markdown(
    "Trains a PPO RL agent to hedge a European Call Option. "
    "Compare against naive delta hedging and no-hedge baseline."
)
...
with tab1:
    ...
    with col2:
        ...
        for i in range(n_paths_show):
            fig2.add_trace(go.Scatter(
                x=days, y=all_options[:, i],
                mode="lines", line=dict(width=1),
                opacity=0.7, showlegend=(i == 0),
                name="Option price",
            ))
        fig2.update_layout(
            title="Option Prices (Black-Scholes)",
            xaxis_title="Trading Day",
    ...
    fig3 = px.histogram(
        x=final_prices, nbins=60,
        labels={"x": "Final Stock Price (₹)"},
        title=f"Distribution of Final Stock Prices (200 paths)",
        color_discrete_sequence=["#4C72B0"],
    )
    fig3.add_vline(x=K, line_dash="dash", line_color="red",
                   annotation_text=f"Strike K={K}")
    ...
    col3.metric("In-the-money %",    f"{(final_prices > K).mean()*100:.1f}%",
                help="% of paths where stock ended above strike price")
...
with tab2:
    ...
    option_path = all_options[:, path_idx]
    ...
    with col2:
        fig2 = go.Figure()
        fig2.add_trace(go.Scatter(x=days, y=delta_path,
                                  name="True Delta (Naive Hedge)",
                                  line=dict(color="#DD8452", dash="dash", width=2)))
        fig2.add_trace(go.Scatter(x=days, y=rl_path,
...
# ──────────────────────────────────────────────────────
# TAB 3: RL vs Delta — Aggregate Comparison
# ──────────────────────────────────────────────────────
with tab3:
    ...
    fig_heat = px.imshow(
        ...
        title="Delta Heatmap (50 paths × 252 days)",
        aspect="auto",
    )
...
with tab4:
    ...
    st.markdown(
        "See how option price and delta change as parameters vary."
    )
    ...
    with col2:
        ...
        fig2.add_trace(go.Scatter(x=T_range[::-1], y=delta_over_time[::-1],
                                  name="Delta", line=dict(color="#DD8452", width=2, dash="dash")),
                       secondary_y=True)
    ...
    fig3 = go.Figure(data=[go.Surface(z=ZZ, x=sigma_range, y=T_range_2d,
                                      colorscale="Viridis")])
    fig3.update_layout(
        title="Option Price Surface (σ vs Time to Expiry)",
        scene=dict(
